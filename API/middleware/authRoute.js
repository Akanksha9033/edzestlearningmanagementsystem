...
//       sub, email: username, name, role, emailVerified
//     });
...
// const JWKS_LOGIN = createRemoteJWKSet(new URL(`${ISSUER}/.well-known/jwks.json`));
...
//     if (init.ChallengeName === "NEW_PASSWORD_REQUIRED") {
//       if (!newPassword) {
//         return res.status(409).json({ challenge: "NEW_PASSWORD_REQUIRED", message: "NEW_PASSWORD_REQUIRED" });
//       }
...
//     const groupsResp = await cognito.adminListGroupsForUser({
...
//     let sub;
//     try {
//       const { payload } = await jwtVerify(idToken, JWKS_LOGIN, { issuer: ISSUER });
//       sub = payload.sub;
//     } catch {
//       sub = attrs.sub;
//     }

//     const profile = await ensureProfile({ sub, email: username, name, role, emailVerified });
...
// router.get("/sessions", authAccess, async (_req, res) => res.json({ sessions: [] }));
...
//     const { email } = req.body || {};
//     if (!email) return res.status(400).json({ message: "Email required" });
//     await cognito.forgotPassword({ ClientId: APP_CLIENT_ID, Username: String(email).toLowerCase() }).promise();
//     res.json({ message: "Reset code sent" });
//   } catch {
//     res.json({ message: "Reset code sent" });
//   }
...
router.post("/register", tight, async (req, res) => {
  try {
    ...
    return res
      .status(201)
      .json({ message: "Registered. Check your email for the verification code." });
  } catch (e) {
    if (e.code === "UsernameExistsException") {
      return res.status(409).json({ message: "Email already in use" });
    }
  ...
  try {
    ...
    if (init.ChallengeName === "NEW_PASSWORD_REQUIRED") {
      if (!newPassword) {
        return res
          .status(409)
          .json({ challenge: "NEW_PASSWORD_REQUIRED", message: "NEW_PASSWORD_REQUIRED" });
      }
...
router.post("/forgot-password", tight, async (req, res) => {
  try {
    ...
    if (!email) return res.status(400).json({ message: "Email required" });
    await cognito
      .forgotPassword({ ClientId: APP_CLIENT_ID, Username: String(email).toLowerCase() })
      .promise();
    res.json({ message: "Reset code sent" });
  } catch {
    res.json({ message: "Reset code sent" });
