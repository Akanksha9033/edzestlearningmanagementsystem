...
export default function StudentQBankList() {
  const [banks, setBanks] = useState([]);
  const [attempts, setAttempts] = useState({});
  const [enrolledMap, setEnrolledMap] = useState({}); // ✅ enrollment state
    ...
    <Box sx={{ p: 3 }}>
      ...
      <Grid container spacing={2}>
        {visibleBanks.map((bank) => {
          ...
          const isPaid = bank.isPaid === true;
          const isEnrolled = enrolledMap[bankId] === true;
              ...
              >
                ...
                <CardContent sx={{ flexGrow: 1 }}>
                  ...
                  {isPaid && (
                    <Typography
                      variant="body2"
                      sx={{ color: "#4748ac", fontWeight: 600, mt: 0.5 }}
                    >
                      ₹{(bank.pricePaise || 0) / 100}
                    </Typography>
                  )}
                </CardContent>
                ...
                <Box px={2} pb={2}>
                  {!isEnrolled ? (
                    <EnrollButton
                      ...
                      isPaid={isPaid}
                      pricePaise={bank.pricePaise || 0}
                      isEnrolled={false}
                      onEnrolled={fetchEnrollments}
                      onOpen={() => goFilter(bank)}
                    />
                  ) : (
                    <>
                      <Button
