...
import { summarizeSelection as summarizeAPI, getGoogleSuggestions } from "../../../../WebAdmin/AdminModule/EBooks/api/ebooksApi";
import NotesPad from "../../../../WebAdmin/AdminModule/EBooks/Blocks/NotesPad";
        ...
        <table className="student-table">
          <tbody>
            {normalized.map((row, r) => (
              <tr key={r}>
                {row.map((val, c) => {
                  const bold = isBoldMarked(val);
                  const txt = unwrapBold(val);
                  return (
                    <td key={c}>
                      <div
  ...
  return (
    <div className={`relative min-h-screen bg-white text-black ${showNotes && notesMode === "split" ? "notes-open" : ""}`} ref={containerRef}>
      ...
      <style>{`
        ...
        /* InfoBox styles */
        .ibx-wrap{--btn:#4748ac}
        .ibx-card{border:2px solid #1f2937;border-radius:14px;background:#fff;padding:14px;display:flex;align-items:center;justify-content:space-between;gap:10px}
        .ibx-title{font-weight:800;color:#0f172a;margin:0}
        .ibx-sub{font-size:13px;color:#6b7280}
        .ibx-btn{appearance:none;border:none;border-radius:12px;padding:8px 12px;color:#fff;font-weight:800;cursor:pointer;background:var(--btn);box-shadow:0 8px 22px rgba(71,72,172,.25)}
        .ibx-btn.is-close{background:#0f172a}
        .ibx-pop{position:relative;margin-top:10px;border:2px solid #1f2937;border-radius:12px;background:#fff;box-shadow:0 12px 28px rgba(2,6,23,.12);padding:12px}
        .ibx-pop-title{font-weight:800;color:#0f172a;margin:0 0 8px 0}
        .ibx-pop-body{white-space:pre-wrap;line-height:1.65;color:#0f172a}
