...
export default function StudentResult() {
    ...
    (async () => {
      try {
        ...
        const r = await API.get(
          `/api/student/attempts/${attemptId}/summary`,
          { signal: ac.signal }
        );
        if (!ac.signal.aborted) {
          setAttempt(r.data?.attempt || null);
        ...
        if (ac.signal.aborted) return;
        if (e?.response?.status === 401) setAuthErr(true);
        console.error(e);
        alert("Failed to load result");
      } finally {
        if (!ac.signal.aborted) setLoading(false);
  ...
  // ✅ SAFE: derive mockTestId from attempt object (NO logic change)
  const mockTestId =
    attempt?.mockTestId ||
    attempt?.testId ||
    attempt?.mocktestId ||
    ...
    return (
      <Box p={4} textAlign="center">
        ...
        <Button
          variant="text"
          startIcon={<ArrowBackIcon />}
          onClick={() => nav(`/student/attempts/${mockTestId}`)}
        >
          Back
        </Button>
      </Box>
