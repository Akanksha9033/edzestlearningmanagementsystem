...
export default function EBookSettings() {
  ...
  // Cover image stored as a URL (editable text)
  const [coverImage, setCoverImage] = useState("");
    ...
    <div className="max-w-6xl mx-auto p-4 sm:p-6 lg:p-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 mb-6">
        <div className="min-w-0">
          <h1 className="text-xl sm:text-2xl font-bold">E-Book Settings</h1>
          <p className="text-gray-600 text-sm sm:text-base">
            Edit details, publish/unpublish, or delete.
          </p>

          <div className="mt-2 text-xs sm:text-sm text-gray-700 flex flex-wrap items-center gap-x-2 gap-y-1">
            <span className="font-semibold truncate max-w-[260px] sm:max-w-[360px]">
              {title || "Untitled"}
            </span>
            <span className="text-gray-400">•</span>
            <span className="text-gray-500 break-all">/{slug || "—"}</span>
            <span
              className={`ml-0.5 inline-block rounded-full px-2 py-0.5 text-[10px] sm:text-xs font-semibold ${
                status === "PUBLISHED"
                  ? "bg-green-100 text-green-700"
                  : "bg-yellow-100 text-yellow-700"
              }`}
      ...
      {/* Details card */}
      <div className="rounded-xl border border-gray-200 bg-white p-4 sm:p-5 shadow-sm mb-5">
        <div className="flex items-start gap-3">
          <div className="hidden sm:grid mt-1 h-9 w-9 place-items-center rounded-md border border-gray-200">
          ...
          <div className="flex-1 min-w-0">
            <h3 className="text-base sm:text-lg font-semibold">Basic Details</h3>
            <p className="text-xs sm:text-sm text-gray-600">
              Update title, slug, and cover image URL.
            </p>

            <div className="mt-4 grid gap-4 sm:grid-cols-2">
              <div className="min-w-0">
                <label className="block text-[10px] sm:text-xs uppercase text-gray-500 mb-1">Title</label>
                <input
                  className="w-full rounded-lg border border-gray-300 px-3 py-2 outline-none focus:ring-2 focus:ring-indigo-300"
                  value={title}
                  onChange={(e) => setTitle(e.target.value)}
              ...
              <div className="min-w-0">
                <label className="block text-[10px] sm:text-xs uppercase text-gray-500 mb-1">Slug</label>
                <input
                  className="w-full rounded-lg border border-gray-300 px-3 py-2 outline-none focus:ring-2 focus:ring-indigo-300"
                  value={slug}
                  onChange={(e) => setSlug(e.target.value)}
              ...
              {/* Cover image URL (editable) */}
              <div className="sm:col-span-2">
                <label className="block text-[10px] sm:text-xs uppercase text-gray-500 mb-1">
                  Cover Image URL
                </label>
                <input
                  className="w-full rounded-lg border border-gray-300 px-3 py-2 outline-none focus:ring-2 focus:ring-indigo-300 break-all"
                  value={coverImage}
                  onChange={(e) => setCoverImage(e.target.value)}
                  placeholder="https://.../cover.jpg"
                />
                <p className="mt-1 text-[11px] text-gray-500">
                  Paste a full URL (e.g., S3 or CloudFront). Click “Save Changes” to persist.
                </p>
                ...
                {!!coverImage && (
                  <div className="mt-3">
                    <img
                      src={coverImage}
                      alt="Cover preview"
                      className="h-20 w-20 sm:h-24 sm:w-24 object-cover rounded-lg border border-gray-200 shadow-sm"
            ...
            <div className="mt-4">
              <button
                disabled={saving}
                onClick={handleSaveMeta}
                className="w-full sm:w-auto rounded-lg bg-indigo-600 px-4 py-2 text-white hover:bg-indigo-700 disabled:opacity-60"
              >
                {saving ? "Saving…" : "Save Changes"}
              </button>
            </div>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <div className="rounded-xl border border-gray-200 bg-white p-4 sm:p-5 shadow-sm">
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1 min-w-0">
              <h3 className="text-base sm:text-lg font-semibold">Publish E-Book</h3>
              <p className="text-xs sm:text-sm text-gray-600">Publish/Unpublish the e-book for your learners</p>
              <div className="mt-4">
                {status === "DRAFT" ? (
                  <button
                    disabled={saving}
                    onClick={() => publishToggle(true)}
                    className="w-full sm:w-auto rounded-lg bg-purple-700 px-4 py-2 text-white hover:bg-purple-800 disabled:opacity-60"
                  >
                    {saving ? "Publishing…" : "Publish"}
                  </button>
                ) : (
                  <button
                    disabled={saving}
                    onClick={() => publishToggle(false)}
                    className="w-full sm:w-auto rounded-lg bg-gray-700 px-4 py-2 text-white hover:bg-gray-800 disabled:opacity-60"
                  >
                    {saving ? "Updating…" : "Unpublish"}
                  </button>
                )}
        ...
        <div className="rounded-xl border border-gray-200 bg-white p-4 sm:p-5 shadow-sm">
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1 min-w-0">
              <h3 className="text-base sm:text-lg font-semibold">Delete E-Book</h3>
              <p className="text-xs sm:text-sm text-gray-600">Delete your e-book if you no longer require it</p>
              <div className="mt-4">
                <button
                  disabled={saving}
                  onClick={handleDelete}
                  className="w-full sm:w-auto rounded-lg bg-red-600 px-4 py-2 text-white hover:bg-red-700 disabled:opacity-60"
                >
                  {saving ? "Deleting…" : "Delete"}
                </button>
              </div>
