            ...
            {cells.map((row, r) => {
              ...
              return (
                <tr key={r} className={rowSelected ? "tb-row-selected" : undefined}>
                  {row.map((val, c) => {
                    const isBold = isBoldMarked(val);
                    const displayText = unwrapBold(val);
                      ...
                      >
                        {isStudent ? (
                          <div
                            ...
                            style={{ fontWeight: isBold ? 800 : 400 }}
                          >
                            {displayText}
                          </div>
                        ) : (
                          <input
                            ...
                            value={displayText}
                            onChange={(e) => {
                              // Keep current bold state while editing
                              const newPlain = e.target.value;
                              const toStore = isBold ? wrapBold(newPlain) : newPlain;
                              setCell(r, c, toStore);
                            }}
