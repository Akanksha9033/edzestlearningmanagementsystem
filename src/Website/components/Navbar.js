...
const Navbar = () => {
  ...
  // ✅ Learnyst URL: set via env in prod; fallback only for safety
  const LEARNYST_URL =
    process.env.REACT_APP_LEARNYST_URL || "https://exams.edzest.org/learn/account/signin";
      ...
      <div className="container mx-auto px-4 py-2 d-flex justify-content-between align-items-center">
        ...
        <nav className="d-none d-md-flex gap-4 text-gray-700 font-medium align-items-center">
          ...
          {user ? (
            ...
            // ✅ Desktop: Login → Learnyst (keeps /login route intact for manual access)
            <a
              href={LEARNYST_URL}
              target="_blank"
              rel="noopener noreferrer"
        ...
        <div id="mobile-menu" className="mobile-menu d-md-none" role="dialog" aria-modal="true">
          ...
          <nav className="mobile-nav">
            ...
            {user ? (
              ...
              // ✅ Mobile: Login → Learnyst (keeps /login route intact for manual URL)
              <a href={LEARNYST_URL} className="mobile-primary-btn">
                Log in
              </a>
            )}
          </nav>
